...
def date_if(prefer_sit, driver):
    ...
    if times_roll and len(times_roll) > 0:
        ...
        if all(time in full_day_times for time in get_time_range(full_day_times, "16:00", "20:00")):
            day_type = 3
...
def choose_it(driver, sit_avilable, idx, reading_room, day_type, max_attempts=500):
    ...
    bool: 预约是否成功
    """
    dir_time = {3: [['16:00', '20:00'], ['16:00', '20:00']],
            ...
            if reserve_box:
                ...
                if times_roll:
                    ...
                    if all(time in time_texts for time in
                           get_time_range(full_day_times, "16:00", "20:00")):
                        print(f"找到全天可预约座位: {i}")
                        found_full_day = True
                        day_type = 3
...
def main():
    ...
    sit_avilable, day_type = None, None
    users = {"自定义": [account_password4, "三楼智慧研修空间", "50"]}
    user = "自定义"
